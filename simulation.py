...
speeds = {'car':2.25, 'bus':1.8, 'truck':1.8, 'rickshaw':2, 'bike':2.5,'ambulance':3.0}  # average speeds of vehicles
...
def setTime():
    ...
    os.system("echo detecting vehicles, "+directionNumbers[(currentGreen+1)%noOfSignals])
...
def handleAmbulanceCase():
    random_intersection = random.randint(0, noOfSignals-1 )
    
    print(f'Ambulance detected! Turning signal {random_intersection} green.')
    signals[random_intersection].green = 1  # Set green light on
    signals[random_intersection].red = 0    # Turn off red light
    signals[random_intersection].yellow = 0 # Turn off yellow light

    pygame.time.set_timer(pygame.USEREVENT, 500)  # 5 seconds timer to reset
...
class Ambulance(Vehicle):
    def __init__(self, lane, direction_number, direction):
        super().__init__(lane, 'ambulance', direction_number, direction, False)
        self.sound = pygame.mixer.Sound("images/images/ambulance_siren.wav")  # Add a siren sound
...
def trigger_ambulance_event():
    global currentGreen, currentYellow
    # Choose a random direction for the ambulance
    ambulance_direction = random.choice(list(directionNumbers.keys()))
    ambulance_lane = random.randint(0, noOfLanes - 1)
    
    # Create and add the ambulance
    ambulance = Ambulance(ambulance_lane, ambulance_direction, directionNumbers[ambulance_direction])
    simulation.add(ambulance)
    
    # Turn all signals red except for the ambulance's direction
    for i in range(noOfSignals):
...
class Main:
    ...
    background = pygame.image.load('images/images/mod_int.png')
    ...
    redSignal = pygame.image.load('images/images/signals/red.png')
    yellowSignal = pygame.image.load('images/images/signals/yellow.png')
    greenSignal = pygame.image.load('images/images/signals/green.png')
    font = pygame.font.Font(None, 30)

    thread3 = threading.Thread(name="generateVehicles",target=generateVehicles, args=())    # Generating vehicles
    thread3.daemon = True
    thread3.start()
    start_img = pygame.image.load('images/images/start_btn.png').convert_alpha()
    start_button = button.Button(1100, 100, start_img, 1)
    ...
    while True:
        ...
        if start_button.draw(screen):
            print('START')
            defaultSig=trigger_ambulance_event()
            
        for i in range(defaultSig,noOfSignals):  # display signal and set timer according to current status: green, yello, or red
            if(i==currentGreen):
                if(currentYellow==1):
        ...
        for i in range(defaultSig,noOfSignals):  
